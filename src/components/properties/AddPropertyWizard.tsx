...
import { X, ArrowLeft, ArrowRight, Check, Home, User as UserIcon, CreditCard, Building2, UserPlus } from 'lucide-react';
...
export const AddPropertyWizard: React.FC<AddPropertyWizardProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  const [isEstateOwned, setIsEstateOwned] = useState<boolean | null>(null);
  ...
  const [shouldAddTenant, setShouldAddTenant] = useState<boolean | null>(null);
  const [charges, setCharges] = useState<ChargeFormData[]>(defaultCharges);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [isSubmitting, setIsSubmitting] = useState(false);

  const getTabsForFlow = () => {
    const baseTabs = [
      { id: 0, name: 'Property Details', icon: Home },
      { id: 1, name: 'Ownership', icon: Building2 },
    ];

    if (isEstateOwned === false) {
      baseTabs.push({ id: 2, name: 'Landlord Info', icon: UserIcon });
      
      if (shouldAddTenant === true) {
        baseTabs.push({ id: 3, name: 'Tenant Info', icon: UserPlus });
        baseTabs.push({ id: 4, name: 'Service Charges', icon: CreditCard });
      } else if (shouldAddTenant === false) {
        baseTabs.push({ id: 3, name: 'Service Charges', icon: CreditCard });
      }
    } else if (isEstateOwned === true) {
      if (shouldAddTenant === true) {
        baseTabs.push({ id: 2, name: 'Tenant Info', icon: UserPlus });
        baseTabs.push({ id: 3, name: 'Service Charges', icon: CreditCard });
      } else if (shouldAddTenant === false) {
        baseTabs.push({ id: 2, name: 'Service Charges', icon: CreditCard });
      }
    }
  ...
  const tabs = getTabsForFlow();

  const getMaxTabIndex = () => {
    return Math.max(...tabs.map(tab => tab.id));
  };

  const getNextTabId = (currentId: number) => {
  ...
  const getPreviousTabId = (currentId: number) => {
    const currentIndex = tabs.findIndex(tab => tab.id === currentId);
    if (currentIndex > 0) {
      return tabs[currentIndex - 1].id;
    }
  ...
  const validateCurrentTab = () => {
    ...
    if (currentTab === 0) {
      ...
      if (isEstateOwned === null) newErrors.ownership = 'Please select property ownership';
    } else if (currentTab === 2 && isEstateOwned === false) {
      // Landlord validation (only if not estate owned)
      if (!landlordData.name.trim()) newErrors.landlordName = 'Landlord name is required';
      if (!landlordData.email.trim()) newErrors.landlordEmail = 'Landlord email is required';
      if (!landlordData.phone.trim()) newErrors.landlordPhone = 'Landlord phone is required';
      
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (landlordData.email && !emailRegex.test(landlordData.email)) {
        newErrors.landlordEmail = 'Please enter a valid email address';
      }
    } else if ((currentTab === 3 && isEstateOwned === false && shouldAddTenant === true) || 
               (currentTab === 2 && isEstateOwned === true && shouldAddTenant === true)) {
      // Tenant validation (when adding tenant)
  ...
  const handleNext = () => {
    if (validateCurrentTab()) {
      const nextTabId = getNextTabId(currentTab);
      setCurrentTab(nextTabId);
    }
  };

  const handlePrevious = () => {
    const previousTabId = getPreviousTabId(currentTab);
  ...
  const handleLandlordChange = (field: keyof LandlordFormData, value: string) => {
    setLandlordData(prev => ({ ...prev, [field]: value }));
    if (errors[`landlord${field.charAt(0).toUpperCase() + field.slice(1)}`]) {
      setErrors(prev => ({ ...prev, [`landlord${field.charAt(0).toUpperCase() + field.slice(1)}`]: '' }));
    }
  };
  ...
  const handleOwnershipChange = (owned: boolean) => {
    setIsEstateOwned(owned);
    // Reset dependent states when ownership changes
    setShouldAddTenant(null);
    if (errors.ownership) {
      setErrors(prev => ({ ...prev, ownership: '' }));
    }
  };

  const handleTenantDecision = (addTenant: boolean) => {
    setShouldAddTenant(addTenant);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const completeData = {
        ...
        isEstateOwned,
        landlord: isEstateOwned ? null : landlordData,
        tenant: shouldAddTenant ? tenantData : null,
  ...
  const isLastTab = currentTab === getMaxTabIndex();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
          ...
          <nav className="flex space-x-8 px-6 overflow-x-auto">
            {tabs.map((tab) => {
              ...
              return (
                <button
                  key={tab.id}
                  className={`flex items-center py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-150 whitespace-nowrap ${
                    isActive
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          ...
          {currentTab === 1 && (
            <div className="space-y-6">
              ...
              <p className="text-sm text-gray-600 mb-6">
                Please specify who owns this property to determine the next steps.
              </p>
                ...
                >
                  <div className="flex items-center">
                    <input
                      type="radio"
                      name="ownership"
                      checked={isEstateOwned === true}
                      onChange={() => handleOwnershipChange(true)}
                      className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2"
                    />
                    <div className="ml-4">
                      <h4 className="text-lg font-medium text-gray-900">Yes, owned by the estate</h4>
                      <p className="text-sm text-gray-600 mt-1">
                        This property is directly owned and managed by the estate administration.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                >
                  <div className="flex items-center">
                    <input
                      type="radio"
                      name="ownership"
                      checked={isEstateOwned === false}
                      onChange={() => handleOwnershipChange(false)}
                      className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2"
                    />
                    <div className="ml-4">
                      <h4 className="text-lg font-medium text-gray-900">No, owned by a private landlord</h4>
                      <p className="text-sm text-gray-600 mt-1">
                        This property is owned by an individual landlord. We'll create a landlord account for them.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {/* Tenant Decision for Estate-Owned Properties */}
              {isEstateOwned === true && (
                <div className="mt-8 pt-6 border-t border-gray-200">
                  <h4 className="text-md font-medium text-gray-900 mb-4">Would you like to add a tenant now?</h4>
                  <div className="flex space-x-4">
                    ...
                    >
                      ...
                      <div className="text-sm font-medium">Add Tenant Now</div>
                    </button>
                    <button
          ...
          {currentTab === 2 && isEstateOwned === false && (
            <div className="space-y-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Landlord Information</h3>
              <p className="text-sm text-gray-600 mb-6">
                We'll create a landlord account with these details. The landlord will receive login credentials to manage their property.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                      errors.landlordName ? 'border-red-300' : 'border-gray-300'
                    }`}
                    placeholder="Enter landlord's full name"
                  />
                  {errors.landlordName && <p className="text-red-600 text-sm mt-1">{errors.landlordName}</p>}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                      errors.landlordEmail ? 'border-red-300' : 'border-gray-300'
                    }`}
                    placeholder="landlord@email.com"
                  />
                  {errors.landlordEmail && <p className="text-red-600 text-sm mt-1">{errors.landlordEmail}</p>}
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                      errors.landlordPhone ? 'border-red-300' : 'border-gray-300'
                    }`}
                    placeholder="+234-123-456-7890"
                  />
                  {errors.landlordPhone && <p className="text-red-600 text-sm mt-1">{errors.landlordPhone}</p>}
                </div>
              </div>

              {/* Tenant Decision for Landlord-Owned Properties */}
              <div className="mt-8 pt-6 border-t border-gray-200">
                <h4 className="text-md font-medium text-gray-900 mb-4">Would the landlord like to add a tenant now?</h4>
                <p className="text-sm text-gray-600 mb-4">
                  The landlord can add tenants now or do it later from their dashboard.
                </p>
                <div className="flex space-x-4">
                  <button
                    onClick={() => handleTenantDecision(true)}
          ...
          {((currentTab === 3 && isEstateOwned === false && shouldAddTenant === true) || 
            (currentTab === 2 && isEstateOwned === true && shouldAddTenant === true)) && (
            <div className="space-y-6">
          ...
          {((currentTab === 4 && isEstateOwned === false) || 
            (currentTab === 3 && isEstateOwned === true) ||
            (currentTab === 3 && isEstateOwned === false && shouldAddTenant === false) ||
            (currentTab === 2 && isEstateOwned === true && shouldAddTenant === false)) && (
            <div className="space-y-6">
